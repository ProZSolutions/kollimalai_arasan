export function getContactAcknowledgementEmailTemplate(params: {
  ...
  const emailSubject = contactSubject
    ? `We have received your message: ${contactSubject} - Kollimalai Arasan`
    : `We have received your message - Kollimalai Arasan`;
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .message-box { background-color: #f3f4f6; border-radius: 8px; padding: 20px; margin: 24px 0; border-left: 4px solid #15803d; }
...
export function getContactReplyEmailTemplate(params: {
  ...
  const emailSubject = originalSubject
    ? `Re: ${originalSubject}`
    : `Response from Kollimalai Arasan`;
    ...
    <html>
      ...
      <body>
        <div class="container">
          ...
          <p style="margin-top: 24px;">Regards,<br><strong>Kollimalai Arasan Team</strong></p>
          <div class="footer">&copy; ${new Date().getFullYear()} Kollimalai Arasan. All rights reserved.</div>
        </div>
      </body>
    </html>
